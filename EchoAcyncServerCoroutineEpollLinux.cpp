...
public:
    ...
    using handle_simple_callback_t   = std::function<void(void)>;
...
//CORO SUPPORT
    ...
    struct ReadAwaiter {
        ...
        bool await_ready() const noexcept { return false; } // Всегда асинхронно
        void await_suspend(std::coroutine_handle<> handle) {
            server.ReadAsync(fd, [this, handle = std::move(handle)](int, std::string result) {
                data = std::move(result); // Сохраняем результат
                handle.resume(); // Возобновляем выполнение
            });
        }
        std::string await_resume() noexcept { return data; } // Возвращаем результат
    ...
    struct WriteAwaiter {
        ...
        void await_suspend(std::coroutine_handle<> handle) const {
            server.WriteAsync(fd, [this, handle = std::move(handle)](int, int) {
                handle.resume(); 
            }, data);
        }
    ...
    struct AcceptAwaiter {
        ...
        bool await_ready() const noexcept { return false; } // Всегда асинхронно
        void await_suspend(std::coroutine_handle<> handle) {
            server.AcceptAsync ([this, handle = std::move(handle)](int fd) {
                fd_ = fd; // Сохраняем результат
                handle.resume(); // Возобновляем выполнение
            });
        }
        int await_resume() noexcept { return fd_; } // Возвращаем результат
    //private: 
        //int fd_;
    };

    auto WriteAsync(int fd, std::string str) {
        return WriteAwaiter{*this, fd, std::move(str)};
    }

    auto ReadAsync(int fd) {
        return ReadAwaiter{*this, fd, ""};
    }

    auto AcceptAsync() {
        return AcceptAwaiter{*this, 0};
    }
